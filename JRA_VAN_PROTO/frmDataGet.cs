...
namespace JRA_VAN_PROTO
{
    public partial class frmDataGet : Form
    {
        ...
        private void LoadInitialData()
        {
            ...
            this.lstDataspec.DataSource = JVUtils.GetDataSpecs();
        ...
        private void btnGetData_Click(object sender, EventArgs e)
        {
            ...
            var dataSpecs = new List<string>();
            foreach (var item in this.lstDataspec.SelectedItems)
            {
                dataSpecs.Add(handler.GetDataSpec(((Parameter)item).Code));
            }
            int optionFlag = ((Parameter)this.cmbOption.SelectedItem).CodeAsInt;
            string fromTime = this.txtFromtime.Text;

            foreach (var dataSpec in dataSpecs) { 
                try
                {
                    // ①JVLinkﾉ初期化(JVInit)
        ...
        private void UpdataDataspecSelection()
        {
            var option = (Parameter)this.cmbOption.SelectedItem;
            var dataSpecs = option.Tag?.Split(",");
            
            for(int i=0; i<lstDataspec.Items.Count; i++)
            {
